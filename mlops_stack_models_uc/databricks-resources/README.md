...
The databricks CLI bundles top level is defined by file `mlops_stack_models_uc/databricks.yml`.
...
ML Resource Configurations in this directory:
 - model workflow (`mlops_stack_models_uc/databricks-resources/model-workflow-resource.yml`)
 - batch inference workflow (`mlops_stack_models_uc/databricks-resources/batch-inference-workflow-resource.yml`)
 - monitoring workflow (`mlops_stack_models_uc/databricks-resources/monitoring-workflow-resource.yml`)
 - feature engineering workflow (`mlops_stack_models_uc/databricks-resources/feature-engineering-workflow-resource.yml`)
 - model definition and experiment definition (`mlops_stack_models_uc/databricks-resources/ml-artifacts-resource.yml`)
...
|-------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------|-------------------------------------|------------------------------------------------|
| dev         | The `dev` environment is used by ML engineers to deploy ML resources to development workspace with `dev` environment configs. The config is for ML project development purposes.                                                           | dev workspace        | dev-mlops-stack-models-uc-model     | /dev-mlops-stack-models-uc-experiment     |
| staging     | The `staging` environment is part of the CD pipeline. Latest main content will be deployed to staging workspace with `staging` environment config.                                                             | staging workspace    | staging-mlops-stack-models-uc-model | /staging-mlops-stack-models-uc-experiment |
| prod        | The `prod` environment is part of the CD pipeline. Latest release content will be deployed to prod workspace with `prod` environment config.                                                                      | prod workspace       | prod-mlops-stack-models-uc-model    | /prod-mlops-stack-models-uc-experiment    |
| test        | The `test` environment is part of the CI pipeline. For changes targeting the main branch, upon making a PR, an integration test will be triggered and ML resources deployed to the staging workspace defined under `test` environment. | staging workspace    | test-mlops-stack-models-uc-model    | /test-mlops-stack-models-uc-experiment    |
...
2. Complete the `TODO` in `mlops_stack_models_uc/databricks.yml` to add the dev workspace URI under `environments.dev.workspace.host`.
...
Following this, update the batch_inference_job base parameters in `mlops_stack_models_uc/databricks-resource/batch-inference-workflow-resource.yml` to pass
...
To get started, open `mlops_stack_models_uc/databricks-resources/batch-inference-workflow-resource.yml`.  The file contains the ML resource definition of a batch inference job, like:
...
resources:
  jobs:
    batch_inference_job:
      name: ${bundle.environment}-mlops-stack-models-uc-batch-inference-job
      tasks:
...
The example above defines a Databricks job with name `${bundle.environment}-mlops-stack-models-uc-batch-inference-job`
that runs the notebook under `mlops_stack_models_uc/deployment/batch_inference/notebooks/BatchInference.py` to regularly apply your ML model for batch inference. 
...
We specify a `batch_inference_job` under `resources/jobs` to define a databricks workflow with internal key `batch_inference_job` and job name `{bundle.environment}-mlops-stack-models-uc-batch-inference-job`. 
...
`test-mlops-stack-models-uc-batch-inference-job`. During the deployment of the `staging` environment config, the job name will be
`staging-mlops-stack-models-uc-batch-inference-job`.
